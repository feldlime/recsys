...
class UserKnn():
    ...
    def _generate_recs_mapper(
                    self,
                    model: ItemItemRecommender, user_mapping: Dict[int, int],
    ...
    def predict(
        ...
        if test.shape[0] == 1:
            recs = pd.DataFrame({'user_id': test['user_id']}, index=test.index)
        else:
