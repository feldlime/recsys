...
1. Помещаем модель в директорию проекта, по-умолчанию для этих целей используется директория `data/models`.
2. В `service/settings.py` добавляем в класс настроек модели путь к файлу модели и название, по которому она будет доступна.
