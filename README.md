...
`models.py` - список используемых моделей.
...
3. Не забываем добавить полученный экземпляр модели в `__all__` - сервис видит только объявленные здесь модели. При попытке обращения к несуществующей/необъявленной модели возникает исключение `ModelNotFoundError`.
