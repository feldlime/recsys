...
class UserKnn():
    ...
    def get_matrix(self, df: pd.DataFrame,
        ...
        self.popular_recs = np.array(df.groupby(item_col).count().sort_values(
            by=user_col, ascending=False).index)
    ...
    def predict(self, test: Union[pd.DataFrame, int], N_recs: int = 10):
        ...
        recs = list(OrderedDict.fromkeys(
            (list(recs[recs['rank'] <= N_recs]['item_id']) + list(self.popular_recs))))[:N_recs]
        return recs
